    ...
    return {
      fetchAlbums: builder.query({ //we are making //!builder.query because we want to get data from the api and we can make //!builder.mutation if we want to send data to the api or delete or update
        providesTags: (result, error, user) => {
          return [{ type: 'Album', id: user.id }] //we use this to refetch the data from the api with specific id not to refetch all the data
      ...
      removeAlbum: builder.mutation({
        invalidatesTags: (result, error, album) => {
          return [{ type: 'Album', id: album.userId }] //we use this to refetch the data from the api with specific id not to refetch all the data
        },
        query: (album) => {
          return {
            url: `/albums/${album.id}`,
